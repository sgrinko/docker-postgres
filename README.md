...
```
Файлы конфигураций используются те же, что и в контейнере Postgres текущей версии. Дополнительно в процессе обновления часть настроек берётся из предыдущей обновляемой версии. Это необходимо делать, так как могут быть некоторые настройки быть критичными для работы контура зависящие от параметров железа на котором мы работаем. На сейчас выполняется перенос следующих параметров:
...
как пример:
```
-v /mnt/bigdrive/postgresql-14:/var/lib/postgresql/14/data\
-v /mnt/bigdrive/postgresql-15:/var/lib/postgresql/15/data\
```
