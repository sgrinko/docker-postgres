...
if ! docker image ls | grep "grufos/pgupgrade" ; then
    echo "    push ..."
    docker tag ${VERSION}_pgupgrade:latest grufos/pgupgrade:latest
    docker push grufos/pgupgrade:latest
    docker tag grufos/pgupgrade:latest grufos/pgupgrade:${VERSION}.${MINOR}
    docker push grufos/pgupgrade:${VERSION}.${MINOR}
fi
